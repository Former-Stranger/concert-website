...
def create_coheadliner_setlists(api_key):
    ...
    for setlist_info in setlists:
        ...
        response = input(f"\n  Create setlist document '{setlist_info['doc_id']}'? (yes/no): ")
        ...
        print(f"  ✓ Created setlist document: {setlist_info['doc_id']}")
    ...
    if old_doc.exists:
        ...
        response = input("  Delete old document? (yes/no): ")
        if response.lower() == 'yes':
    ...
    print("NEXT STEPS")
    print("="*80)
    print("1. Update export_to_web.py to handle multiple setlists per concert")
    print("2. Update concert.js to display multiple setlists")
    print("3. Run: python3 scripts/export_to_web.py")
    print("4. Test: https://earplugsandmemories.com/concert.html?id=847")
    print("5. Deploy: firebase deploy --only hosting")

if __name__ == '__main__':
    if len(sys.argv) < 2:
        print("Usage: python3 create_coheadliner_setlists_847.py YOUR_API_KEY")
        print("\nExample:")
        print("  python3 create_coheadliner_setlists_847.py your-setlistfm-api-key")
        sys.exit(1)
