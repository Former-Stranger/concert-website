        ...
        <div class="vintage-poster rounded-lg p-10 text-center mb-8">
            ...
            ${openers.length > 0 ? `
                <p class="text-xl mb-4 text-[#f4e4c1] opacity-80">
                    with ${openers.map(a => a.name || a.artist_name).join(', ')}
                </p>
            ` : ''}
...
function getArtistNames(artists) {
    ...
    // Use 'name' property (from JSON export) or 'artist_name' (from Firestore)
    return headliners.map(a => a.name || a.artist_name).join(', ') || artists[0].name || artists[0].artist_name;
}
